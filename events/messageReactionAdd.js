...
module.exports = {
	...
	async execute(reaction, user) {
		...
		await member.roles.add(option.roleId, 'Reaction role via PariahBot').catch((error) => {
			console.error(`Failed to add reaction role ${option.roleId} to ${user.id}:`, error.message);
		});
	},
};
