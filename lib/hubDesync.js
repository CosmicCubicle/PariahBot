...
async function handleHubPrune(interaction, guildId, hubChannelId) {
	const removed = voiceStore.removeHub(hubChannelId, guildId);
...
async function handleHubButtonInteraction(interaction) {
	...
	if (action === PRUNE_ACTION) {
		await handleHubPrune(interaction, guildId, hubChannelId);
	} else {
